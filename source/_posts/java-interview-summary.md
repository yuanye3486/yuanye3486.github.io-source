...
### 什么是JVM的类加载机制
  - JVM把描述类的数据从 Class 文件加载到内存，并对数据进行校验、转换解析和初始化，最终形成可以被虚拟机直接使用的java类型，这就是虚拟机的类加在机制。
...
### Spring Bean 依赖注入发生的时机
  - 用户第一次通过 getBean 方法向 IoC 容索要 Bean 时。
  - 当用户在 <Bean> 配置了 lazy-init 属性，即让容器在解析注册 Bean 定义时进行预实例化，触发依赖注入。
...
### [Spring 如何解决 Bean 的循环依赖]（https://blog.csdn.net/crazyzhb2012/article/details/53353552）
 - 循环依赖定义
...
### Session 分布式方案
### Session 分布式处理
### 分布式锁的应用场景、分布式锁的产生原因、基本概念、实现
  + 概念
    分布式锁：当多个进程不在同一个系统中，用分布式锁控制多个进程对资源的访问。
  + 应用场景
    如果是分布式情况下（多JVM），线程A和线程B很可能不是在同一JVM中，这样线程锁就无法起到作用了，这时候就要用到分布式锁来解决。
  + [实现方式](https://blog.csdn.net/zdy0_2004/article/details/53070209)
    + 基于数据库实现分布式锁
      要实现分布式锁，最简单的方式可能是直接创建一张锁表，然后通过操作该表中的数据来实现。
      - 当我们要锁住某个方法或资源时，就在该表中增加一条记录，想要释放锁的时候就删除这条记录。
        + 缺点
          1. 该锁强依赖数据库的可用性，数据库是一个单点，一旦数据库挂掉，会导致业务系统不可用。
          2. 该锁没有失效时间，一旦解锁操作失败，就会导致锁记录一直在数据库中，其他线程无法再获得到锁。
          3. 该锁只能是非阻塞的，因为数据的insert操作，一旦插入失败就会直接报错。没有获得锁的线程并不会进入排队队列，要想再次获得锁就要再次触发获取锁操作。
